...
public class FileManager {
	...
	public List<String> readLinesFromFile(String filename) throws PersistenceException {
		...
		try {
			...
			LogHandler.getInstance().log(Level.SEVERE, FileManager.class.getName(), "readLinesFromFile",
					e.getLocalizedMessage(), e);
			throw new PersistenceException(String.format("File not found (%s)", filename));
		} catch (IOException e) {
			LogHandler.getInstance().log(Level.SEVERE, FileManager.class.getName(), "readLinesFromFile",
					e.getLocalizedMessage(), e);
			throw new PersistenceException(String.format("Could not read from file (%s)", filename));
		}
		return lines;
	}
